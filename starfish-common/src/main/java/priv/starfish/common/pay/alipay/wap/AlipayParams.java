...
public class AlipayParams {
	...
	// 签名方式 ，选择项：0001(RSA)、MD5 <br/>
	// 无线的产品中，签名方式为rsa时，sign_type需赋值为0001而不是RSA
	public static final String sign_type = "MD5";

	// 支付宝提供给商户的服务接入网关URL(新)
	public static final String ALIPAY_GATEWAY = "http://wappaygw.alipay.com/service/rest.htm?";

	// 接口名称
	// call_back_url sec_id
	// 获得授权令牌 //远程模拟 form post
	// 返回数据 URLDECODE res_data xml<-request_token
	//private final String serviceAuth = "alipay.wap.trade.create.direct";
	// get
	// 返回数据纯URL带参数的模式 notify_data xml 日志形式记录
	private final String serviceTrade = "alipay.wap.auth.authAndExecute";
	...
	/**
	 * 传递请求出错时的通知页面路径error_notify_url（需要联系支付宝开通该参数权限）<br/>
	 * 
	 */
	...
	private Map<String, String> getTmpParamMap() {
		...
		tmpParams.put("service", serviceTrade);
	...
	public static String buildRequestSign(Map<String, String> params) {
		String paramStr = AlipayCore.createParamString(params); // 把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串

...
public class NotifyController extends BaseController {
	 ...
	 * 
	 * @author 毛智东
	 * @date 2015年7月8日 下午8:07:37
	 * 
	 * @param mailVerfCode
	 *            页面参数：ajax.data({email: "123456@qq.com", vfCode : "123456"});
	 * @return
	 */
	@Remark("邮箱验证码验证")
	@RequestMapping(value = "/mail/verf/code/valid", method = RequestMethod.POST)
	@ResponseBody
	public Result<Boolean> validMailVerfCode(@RequestBody MailVerfCode mailVerfCode) {
		Result<Boolean> result = Result.newOne();
		result.data = settingService.validMailVerfCode(mailVerfCode);
		return result;
	}

...
<script type="text/javascript">
	...
	function doTheQuery() {
		...
		// ajax post 请求...
		var ajax = Ajax.post("/user/svc/coupon/list/get");
		ajax.data(postData);
	...
	function deleteUserCoupon(id) {
		var ajax = Ajax.post("/user/svc/coupon/deleted/do");
		var postData = {
			id : id
		};
		ajax.data(postData);
...
<script type="text/html" id="couponRowTpl">
	...
	{{# if(!isNoB(coupons)){ }}
		...
		<dd>{{coupon.svcName}}</dd>
		</dl>
		<dl class="labels labels1">
		<dt><label>说明：</label></dt>
		{{#　if(coupon.type=='nopay'){ }}
			<dd>凭此券免费兑换{{coupon.svcName || ""}}</dd>
		{{#　} }}
		{{#　if(coupon.type=='sprice'){ }}
			<dd>凭此券可购买{{coupon.svcName || ""}}只需{{coupon.price}}元</dd>
		{{#　} }}
		{{#　if(coupon.type=='deduct'){ }}
			<dd>凭此券可抵扣{{coupon.svcName || ""}}{{coupon.price}}元</dd>
		{{#　} }}
		</dl>
		</div>
		</div>
		...
		<td>
			{{# if(coupon.invalid==false){ }}
				<input class="btn-normal btn-w70 mb10" type="button" value="立即使用" onclick="msgInfo({{coupon.svcId}})" /><br/>
			{{# } }}

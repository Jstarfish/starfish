...
public class AlipayNotify {
	...
	public static boolean verify(Map<String, String> params) {
		...
		boolean isValidSign = isSameSign(params, sign);

		// 写日志记录（若要调试，请取消下面两行注释）
		String sWord = "Web verify:: responseTxt=" + responseTxt + "\n isValidSign=" + isValidSign + "\n 返回回来的参数：" + AlipayCore.createParamString(params);
		logger.debug(sWord);
		//
	 ...
	 * @return 生成的签名结果
	 */
	private static boolean isSameSign(Map<String, String> params, String sign) {
		// 过滤空值、sign与sign_type参数
		Map<String, String> paramsNew = AlipayCore.filterParams(params);
		// 获取待签名字符串
		String preSignStr = AlipayCore.createParamString(paramsNew);
		// 获得签名验证结果
		return MD5.verify(preSignStr, sign, AlipayParams.key, AlipayParams.inputCharSet);
	}
	...
	private static String verifyResponse(String notify_id) {
		...
		String partner = AlipayParams.parterId;

		String veryfy_url = AlipayParams.verifyUrl + "partner=" + partner + "&notify_id=" + notify_id;

...
import priv.starfish.mall.web.base.AppBase.CacheKeys;
...
/**
	...
	public static List<Region> getRegionsByParentId(Integer parentId) {
		...
		//Cache<String, Object> regionListCache = null;
		SettingService settingService = WebEnvHelper.getSpringBean(SettingService.class);
		if (parentId == -1) {
			// 获取第一级地区/省
			@SuppressWarnings("unchecked")
			List<Integer> rootIds = (List<Integer>) regionListCache.get(CacheKeys.ROOT_IDS);
			...
			if (rootIds == null || rootIds.size() == 0) {
				...
				//
				regionListCache.put(CacheKeys.ROOT_IDS, rootIds);
			} else {

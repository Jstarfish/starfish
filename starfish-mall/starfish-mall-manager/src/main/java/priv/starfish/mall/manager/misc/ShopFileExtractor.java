...
public class ShopFileExtractor implements FileReceiver {
	...
	private static final String TargetUsage = "shop.extract";
	...
	List<TypedField> setXlsColums() {
		...
		col = new TypedField("是否自营", "bool", "是/否");
		columns.add(col);

		col = new TypedField("店铺名称", "str");
		columns.add(col);

		col = new TypedField("经营领域", "str");
		columns.add(col);
	...
	Map<String, String> setMerchColums() {
		...
		columns.put("备注", "memo");
		columns.put("是否自营", "selfFlag");
		columns.put("店铺名称", "name");
		columns.put("经营领域", "bizScope");
		return columns;
	}

	public ShopService getShopService() {
		return WebEnvHelper.getSpringBean(ShopService.class);
	}
}

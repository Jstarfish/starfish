		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					if (ts.firstElementChild) {
						var tmpJqDom = $(ts.firstElementChild);
						tmpJqDom.html(tmpJqDom.html() + rowData.join('')); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
					} else {
						$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join(''));
					}
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if ( !ts.p.multikey ) {
				...
				if(e[ts.p.multikey]) {
					...
					$(ts).jqGrid("setSelection",ri,true,e); //guoyn add:设置ctrlKey后，选择复选框无效
				}
			}

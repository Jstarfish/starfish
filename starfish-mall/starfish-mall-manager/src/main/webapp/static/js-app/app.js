...
function getServerTime(xtra){
	...
	//
	if(xtra == "asTs"){ 
		ajax.params({asTs : true});
	}
	else if(xtra == "asNum"){ 
		ajax.params({asNum : true});
	}
	//
	ajax.sync();
...
function RepoImageDlg() {
	...
	var _hintHtml = '<span style="float:left;padding-left:15px;line-height:40px;color:red;" name="imagex-hint"></span>';
	var _initFlag = false;
...
function bindRegionLists(provinceSelId, citySelId, countySelId, townSelId, initVals) {
	...
	//
	if(!$id(provinceSelId).data("cascadeEventBound")){
		$id(provinceSelId).data("cascadeEventBound", true);
	...
	//
	if(!$id(citySelId).data("cascadeEventBound")){
		$id(citySelId).data("cascadeEventBound", true);
	...
	//
	if(!$id(countySelId).data("cascadeEventBound")){
		$id(countySelId).data("cascadeEventBound", true);

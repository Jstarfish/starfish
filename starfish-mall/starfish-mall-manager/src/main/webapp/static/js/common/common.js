...
/**
 * { "String": "[object String]", "Number": "[object Number]", "Boolean": "[object Boolean]", "Date": "[object Date]", "Array": "[object Array]", "Function": "[object Function]", "RegExp": "[object
 ...
 * @returns {Boolean}
 */
function strStartsWith(srcStr, chkStr, bIgnoreCase) {
 ...
 * @returns {Boolean}
 */
function strEndsWith(srcStr, chkStr, bIgnoreCase) {
	if (!isString(srcStr) || !isString(chkStr)) {
		return false;
	}
	bIgnoreCase = bIgnoreCase == true;
...
/**
 * check whether <b>srcStr</b> contains <b>chkStr</b> with optional <b>bIgnoreCase</b> param
 *
 * @param {String}
 *            srcStr
 * @param {String}
 ...
 * @returns {Boolean}
 */
function strContains(srcStr, chkStr, bIgnoreCase) {
	if (!isString(srcStr) || !isString(chkStr)) {
		return false;
	}
	bIgnoreCase = bIgnoreCase == true;
...
String.prototype.startsWith = function(chkStr, bIgnoreCase) {
	return strStartsWith(this, chkStr, bIgnoreCase);
};
/**
 * extend String method
 *
...
String.prototype.endsWith = function(chkStr, bIgnoreCase) {
	return strEndsWith(this, chkStr, bIgnoreCase);
};
/**
 * extend String method
 *
...
String.prototype.contains = function(chkStr, bIgnoreCase) {
	return strContains(this, chkStr, bIgnoreCase);
};
...
/**
 * 乘法运算 说明：javascript的乘法结果会有误差，在两个浮点数相乘的时候会比较明显。这个函数返回较为精确的乘法结果。
 *
 * @author 邓华锋
 * @date 2015年11月27日 下午4:14:52
 *
 * @param multiplicand
 * @returns {Number}
 */
Number.prototype.multiply = function(multiplicand) {
	var arg1 = this;
	var arg2 = multiplicand;
...
/**
 * 除法运算 说明：javascript的除法结果会有误差，在两个浮点数相除的时候会比较明显。这个函数返回较为精确的除法结果。
 *
 * @author 邓华锋
 * @date 2015年11月27日 下午4:15:01
 *
 * @param divisor
 * @returns {Number}
 */
...
/**
 * 加法运算 说明：javascript的加法结果会有误差，在两个浮点数相加的时候会比较明显。这个函数返回较为精确的加法结果。
 *
 * @author 邓华锋
 * @date 2015年11月27日 下午4:13:42
 *
 * @param augend
 * @returns {Number}
 */
Number.prototype.add = function(augend) {
	var arg1 = this;
	var arg2 = augend;
	var r1, r2, m;
	try {
	...
	return (arg1.multiply(m) + arg2.multiply(m)) / m;
}
/**
 * 减法运算 说明：javascript的减法结果会有误差，在两个浮点数相加的时候会比较明显。这个函数返回较为精确的减法结果。
 *
 * @author 邓华锋
 * @date 2015年11月27日 下午4:14:35
 *
 * @param subtrahend
 * @returns {Number}
 */
Number.prototype.subtract = function(subtrahend) {
...
function setPageUrl(url, target, beforeOpen) {
	if ( typeof target == "string") {
		...
		var dom = getById(__hidden_link_id_for_page);
		if (dom == null) {
...
function getFormElementsByName(formId, elName) {
	...
	var retElems = [];
	var elems = copyAsArray(theForm.elements);
...
var __mailboxHomeUrls = {
	...
	// tom邮箱
	"@tom.com" : "http://listener.mail.tom.com/webmail/login/index.action",
	// 21cn邮箱

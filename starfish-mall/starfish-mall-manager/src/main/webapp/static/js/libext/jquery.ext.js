			...
			if ( typeof _beforeSendCallback == "function") {
				ajaxConf.beforeSend = function(jqXHR) {
					if ( typeof Ajax.beforeSend == "function") {
						Ajax.beforeSend(jqXHR);
					}
					_beforeSendCallback(jqXHR);
				};
			} else if ( typeof Ajax.beforeSend == "function") {
				ajaxConf.beforeSend = function(jqXHR) {
					Ajax.beforeSend(jqXHR);
				};
			}
...
// 隐藏的表单
declare("HiddenForm.core");
HiddenForm.core.fn = function() {
	var _formDom = null;
	var _formId = "form-" + genUniqueStr();
	var _formName = null;
...
HiddenForm.newOne = function() {
	return new HiddenForm.core.fn();
};
// 文件下载---------------------------------------------------------
...
function ContextMenu() {
	if (!$(document).data("__contextmenu_disabled")) {
		$(document).data("__contextmenu_disabled", true);
		//
...
FormProxy.core.fn = function() {
	...
	this.addField = function(fieldInfo) {
		...
		if (idOrName) {
			...
			//
			var idOrName = key = fieldInfo.key;
			fieldsKeyMap.set(key, idOrName);
			fieldsInfoMap.set(idOrName, fieldInfo);
		}
		//
	...
	this.hideMessages = function() {
		...
		//
		idOrNames = fieldsInfoMap.keys();
		for (var i = 0, len = idOrNames.length; i < len; i++) {
			var idOrName = idOrNames[i];
			var fieldInfo = fieldsInfoMap.get(idOrName);
			if (fieldInfo && typeof (fieldInfo.onHideMessage) == "function") {
				fieldInfo.onHideMessage(idOrName);
			}
		}
		//
		return this;

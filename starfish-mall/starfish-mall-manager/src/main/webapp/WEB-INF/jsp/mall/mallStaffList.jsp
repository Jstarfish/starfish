...
<body id="rootPanel">
		...
		<div class="filter section">
			<div class="filter row">
				...
				<div style="float:right;" class="group align right">
					...
					<button id="queryMallStaffBtn" class="button">查询</button>
				</div>
			</div>
		</div>
		<!--店铺人员-角色Grid -->
	...
	<div id="addStaffDialog" style="display: none;">
		<div class="ui-layout-center">
			...
			<table id="mallUserGrid"></table>
			<div id="mallUserPager"></div>
		</div>
    </div>
    ...
    //
    var mallStaffGridCtrl = null;
    //
    var mallUserGridCtrl = null;
    //
    var addMallStaffDialog = null;
    //
	...
	$(function() {
		...
		//绑定查询人员事件
		$id("queryMallStaffBtn").click(function(){
			var filter = {};
			...
			//
			mallStaffGridCtrl.jqGrid("setGridParam", {postData :{filterStr : JSON.encode(filter,true)},page:1}).trigger("reloadGrid");
		...
		$id("btnQueryUser").click(function(){
			...
			mallUserGridCtrl.jqGrid("setGridParam", {postData :{filterStr : JSON.encode(filter,true)},page:1}).trigger("reloadGrid");
		});
	...
	function editStaff(staffId, userRoleListId){
		...
		//
		var ajax = Ajax.post("/mall/user/roles/update/do");
	...
	function loadAllRoles(){
		var ajax = Ajax.post("/mall/roles/and/perms/get");
	...
	//
	function clearaddMallStaffDialog(){
		$id("queryUserName").val("");
		$id("queryUserPhoneNo").val("");
		mallUserGridCtrl.jqGrid("clearGridData");
	}
	//
	function goAddStaff(){
		method = "add";
		clearaddMallStaffDialog();
		addMallStaffDialog.dialog("open");
	}
	//
	...
	function goUnbindUserRoles(staffId){
		var theLayer = Layer.confirm('确定要解除所有角色吗？', function(){
			...
			var ajax = Ajax.post("/mall/user/roles/unbind/do");
	...
	function loadMallStaffData(){
		...
		//
		mallStaffGridCtrl= $("#mallStaffGrid").jqGrid({
		      url : getAppUrl("/mall/staff/list/get"),  
	...
	function loadUserData(){
		...
		//
		mallUserGridCtrl= $("#mallUserGrid").jqGrid({
		      url : getAppUrl("/mall/user/list/get"),  
		      contentType : 'application/json',  
		      mtype : "post",  
		      datatype : 'json',
		      postData:{filterStr : JSON.encode(filter,true)},
	...
	function initAddStaffDialog(){
		addMallStaffDialog = $( "#addStaffDialog" ).dialog({
	        autoOpen: false,
	        ...
	        buttons: {
	            "关闭": function() {
	            	addMallStaffDialog.dialog( "close" );
	          }
	        },
	        close: function() {
	        	addMallStaffDialog.dialog( "close" );
	...
	function adjustCtrlsSize() {
		...
		var gridCtrlId = "mallStaffGrid";
		var jqGridBox = $("#gbox_" + gridCtrlId);
		var headerHeight = $("> div.ui-jqgrid-view > div.ui-jqgrid-hdiv", jqGridBox).height();
		var pagerHeight = $id("mallUsePager").height();
		mallStaffGridCtrl.setGridWidth(mainWidth - 1);
		mallStaffGridCtrl.setGridHeight(mainHeight - headerHeight - pagerHeight - 3 - 90);
	}

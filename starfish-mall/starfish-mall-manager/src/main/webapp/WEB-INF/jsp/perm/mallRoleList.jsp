...
<body id="rootPanel">
	...
	<script type="text/javascript">
		...
		nameMap.add("admin", "商城管理员");
		nameMap.add("servant", "商城客服");
		//权限最初始状态列表
		...
		var tempRoleId = null;
		//缓存当前jqGrid数据行数组
		var mallRoleGridHelper = JqGridHelper.newOne("");
		//唯一标识
		var uniqFlag = true;
		...
		function loadData() {
			roleGrid = $id("roleList")
					.jqGrid(
							{
								url : getAppUrl("/perm/roles/list/get/-mall"),
								...
								colModel : [
										...
										},
										{
											...
											formatter : function(cellValue,
													option, rowObject) {
												return nameMap.get(cellValue) == null ? cellValue
														: nameMap
																.get(cellValue);
											}
										},
								...
								loadComplete : function(gridData) { // JqGridHelper缓存最新的grid数据
									mallRoleGridHelper.cacheData(gridData);
								...
								ondblClickRow: function(rowId) {
									var userMap = mallRoleGridHelper.getRowData(rowId)
		...
		function initDialogAdd() {
			$("div[aria-describedby='roleDialog']").find(
					"span[class='ui-dialog-title']").html("新增角色");
			//
		...
		function deleteBatch() {
			...
			if (ids == "") {
				...
				if (postData.contains(2)) {
					Toast.show("商城管理员不能被删除！");
					return;
				}
		...
		function adjustCtrlsSize() {
			...
			roleGrid.setGridHeight(mainHeight - headerHeight - pagerHeight - 3 - 60);
		}
...
<script id="deliveryTpl" type="text/html">
	<div class="form" style="margin-bottom: 8px;">
		...
		<div class="field row" style="height:80px;">
			...
			<textarea class="field value three wide" maxlength="250" id="desc" style="height:80px;"></textarea>
		</div>
	</div>

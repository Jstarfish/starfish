...
<body id="rootPanel">
		...
		<div class="filter section">
			<div class="filter row">
				<div class="group right aligned">
					...
					<input id="queryRefundStatus" class="input" type="hidden" value="4"/> 
					<span class="spacer"></span>
					<!-- 起止，结束时间 -->
	...
	//-------------------------------------------自定义方法------------------------------{{
		...
		//申请退款 确认提示框
		function toRefundConfirm(id,jqGridCtrl) {
			//
			var yesHandler = function(layerIndex) {
				theLayer.hide();
				toRefund(id,jqGridCtrl);
			};
			var noHandler = function(layerIndex) {
				theLayer.hide();
			};
			//
			var theLayer = Layer.confirm("您确定要申请退款吗？", yesHandler, noHandler);
		}
		//更改支付记录为 退款申请 状态
		function toRefund(id,jqGridCtrl){
			var ajax = Ajax.post("/settle/refund/apply/do");
			ajax.data({
				id : id,
				refundStatus : 6
			});
	...
	$id("mainPanel").jgridDialog(
			{
				...
				jqGridGlobalSetting:{
					url : getAppUrl("/settle/sale/refund/apply/list/get"),
					postData : {

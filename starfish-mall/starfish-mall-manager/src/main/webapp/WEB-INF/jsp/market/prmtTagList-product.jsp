...
<body id="rootPanel">
	...
	//
	function deletePrmtTag(tagId, productId){
		var hintBox = Layer.progress("正在保存数据...");
		//
		var postData = { tagId:tagId, productId:productId };
	  	var ajax = Ajax.post("/goods/product/prmtTag/delete/do");
	  	    ajax.data(postData);
	...
	function goDeletePrmtTag(tagId){
		...
		theLayer= Layer.confirm('确定要删除该促销标签吗？', function(){
			theLayer.hide();
			deletePrmtTag(tagId, currentProductId);
		});
	}
	
	//
	function addPrmtTag(tagId, productId){
		var hintBox = Layer.progress("正在保存数据...");

		var ajax = Ajax.post("/goods/product/prmtTag/add/do");
		//
		ajax.data({productId:productId, prmtTagId:tagId});
	...
	function goAddPrmtTag(tagId){
		...
		//
		addPrmtTag(tagId, currentProductId);
	}
	...
	function queryPrmtTags(){
		...
		//
		filter.productId = currentProductId;
		//

		...
		<div id="addForm" class="form">
			...
			<div class="field row">	
				<label class="field label one half wide required">类型：</label> 
				 <div id="couponType" class="field group">
	...
	<script>
		...
		function isExistByName(name) {
			if (name == oldName) {// 验证是否跟原名称一样，如果一样则跳过验证
				...
				var ajax = Ajax.post("/market/coupon/exist/by/name");
				ajax.data({
					name : name
				});
				ajax.sync();
		...
		function updateDisabledById(jqGridCtrl,obj){
			var ajax = Ajax.post("/market/coupon/disabled/update/do");
		...
		function updateDeletedById(jqGridCtrl,obj){
			var ajax = Ajax.post("/market/coupon/deleted/update/do");
		...
		function showTargetInfo(targetInfo) {
			$id("productName").html("你关联啦"+targetInfo.targetName+"：<span style='color:red'>"+targetInfo.title+"</span>");
			$id("productId").val(targetInfo.id);
			...
			$id("mainPanel").jgridDialog(
					{
						...
						addUrl : "/market/coupon/add/do",
						updUrl : "/market/coupon/update/do",
					...
					jqGridGlobalSetting : {
						...
						modAndViewInit : function(data) {
							...
							textSet("productId", data.productId);
							...
							$id("couponType").find(":input").prop("disabled", true);
							$id("productName").html("关联商品名称："+data.productName || "");
							$id("showTargetInfo").show();
						},
						/**
						 * 封装查询参数
						...
						formProxyInit : function(formProxy) {
							...
							formProxy.addField({
								...
								rules : [ {
									rule : function(idOrName, type, rawValue, curData) {
										// 若显示且为空，给予提示
										if (!$id("productId").is(":visible") && isNoB(rawValue)) {
											Layer.warning("亲，要绑定商品哦！");
											return false;
										}
										return true;
									}
						...
						pageLoad : function(jqGridCtrl,cacheDataGridHelper) {
							...
							couponId.click(function() {
								...
								$id("mainPanel").show();
								jqGridCtrl.jqGrid("setGridParam", {url : getAppUrl("/market/coupon/list/get"),page:1}).trigger("reloadGrid");
							});
							//点击商品弹出模态框
							$id("goods").click(function() {
								var diaLogInfo={
											"title":"选择商品",
											"id":"goodsInfo",
											"src":"<%=appBaseUrl%>/market/coupon/bind/goods/jsp"
											};
								diaLog(diaLogInfo);
		...
		function recyceGrid() {
			recycleGridCtrl= $("#recycleGridCtrl").jqGrid({
			      url : getAppUrl("/market/coupon/recycle/list/get"),  

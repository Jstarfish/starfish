...
<body id="rootPanel">
	...
	function goDisabled(id, disabled){
		//disabled==false,当前由启用改为禁用
		if(!disabled){
			var theLayer = Layer.confirm('确定要禁用该会员吗？', function() {
				theLayer.hide();
				disabledById(id, disabled);
			});
		//disabled==true,当前由禁用改为启用
		}else{
			disabledById(id, disabled);
		}
	}
	
	function disabledById(id, disabled){
		//
		var hintBox = Layer.progress("正在修改...");
		var ajax = Ajax.post("/member/update/status/by/id/do");
		ajax.data({id:parseInt(id), disabled:!disabled});
		ajax.done(function(result, jqXhr) {
			if (result.type == "info") {
				Layer.msgSuccess(result.message);
				//重新加载
	...
	function loadData(){
		...
		jqGridCtrl= $("#member_list").jqGrid({
		      ...
		      colModel : [{name:"id", index:"id", width:50, align : 'right',},
		                  ...
		                  {name:"gradeVO.name", index:"gradeVO.name",width:100,align : 'left'},
		                  {name:"user.phoneNo", index:"user.phoneNo",width:80,align : 'right'},
		                  ...
		                  {name:'id',index:'id', formatter : function (cellValue,option,rowObject) {
							...
							+ cellValue
							+ ")' >修改</a>]  [<a class='item' href='javascript:void(0);' onclick='goDisabled("
							+ cellValue + "," + disabled
							+ ")' >"+disabledStr+"</a>] ";
						},
...
<div class='form'>
	...
	<div class='field row'>
		<div class='field group'>
			...
			<input type='text' id='phoneNo' class='field value' />
		</div>
		<span style='width: 80px;' class='normal spacer'></span>
		<div class='field group'>

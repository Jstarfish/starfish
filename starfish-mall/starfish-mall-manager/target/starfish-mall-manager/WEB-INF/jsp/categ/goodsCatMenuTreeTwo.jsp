...
<div class="ui-layout-center" style="padding: 0;" id="mainPanel">
	<div class="ui-layout-west"	style="padding: 0;" id="secondLevelMenuTree">
		<div class="form">
			<div class="field row" style="background-color: #EFEFEF;">
				<div class="normal group left aligned">
					<label class="field label">二级菜单项</label>
				</div>
				<div class="normal group right aligned">
					<button class="normal button" id="btnAddSecondLevelMenuItem">添加</button>
				</div>
			</div>
			<input type="hidden" id="curSecondLevelMenuItemId" />
			<div style="padding: 4px;" id="secondLevelMenuItem"></div>
		</div>
	</div>
...
<!-- 二级菜单项链接 -->
<div class="form" id="secondLevelMenuItemDialog">
	<div class="field row">
		<label class="field label required">名称</label>
		<input class="field value" type="text" id="secondLevelMenuItemName" />
	</div>
	<div class="field row">
		<label class="field label">包含链接</label>
		<div class="field group">
			<input id="secondLevelMenuItemHasUrlYes" type="radio" name="secondLevelMenuItemHasUrl" value="1" />
			<label for="secondLevelMenuItemHasUrlYes">是</label>
			<input id="secondLevelMenuItemHasUrlNo" type="radio" name="secondLevelMenuItemHasUrl" value="0" checked="checked" />
			<label for="secondLevelMenuItemHasUrlNo">否</label>
		</div>
	</div>
	<div class="field row">
		<label class="field label">URL</label>
		<input class="field value two half wide" type="text" id="secondLevelMenuItemUrl" />
	</div>
	<div class="field row">
...
<script type="text/javascript">
	...
	// 一级菜单项对话框、二级菜单项对话框、三级菜单项Grid、三级菜单项下的关联分类dialog
	var firstLevelMenuItemDialog, secondLevelMenuItemDialog, thirdLevelMenuItemGrid, goodsCatTreeDialog;
	// 一级、二级、三级菜单项表单代理
	var firstLevelMenuItemProxy = FormProxy.newOne();
	var secondLevelMenuItemProxy = FormProxy.newOne();
	var thirdLevelMenuItemProxy = FormProxy.newOne();
	// 当前一级菜单项id、当前二级菜单项id
	var curFirstLevelMenuItemId, curSecondLevelMenuItemId;
	// 关联分类树
	var catTree;
	// 模拟数据计数器（后续要改）
	var count = 0;
	...
	function saveFirstLevelMenuItem() {
		...
		ajax.done(function(result, jqXhr) {
			if (result.type == "info") {
				...
				//loadSecondLevelMenuItems(result.data.id);
				firstLevelMenuItemDialog.dialog("close");
			} else {
				Layer.msgWarning(result.message);
			}
	...
	// 加载商品分类菜单下一级菜单项关联的二级菜单项列表
	function loadSecondLevelMenuItems(pId, _this) {
		$("#firstLevelMenuItem .lable.active").each(function(i,obj){
			$(obj).removeClass("lable active");
		});
		$(_this).parent().addClass("lable active");
		
		$id("secondLevelMenuTree").show();
		$id("thirdLevelMenuItemPanel").hide();
		thirdLevelMenuItemProxy.hideMessages();
		
		// 加载二级菜单项列表
		var ajax = Ajax.post("/categ/menu/items/second/level/get");
		ajax.data({
			parentId : pId
		});
		ajax.done(function(result, jqXhr) {
			if (result.type == "info") {
				$id("secondLevelMenuItem").html(laytpl($id("secondLevelGoodsCategMenuTpl").html()).render(result.data));
			} else {
				Layer.msgWarning(result.message);
			}
		});
	...
	// 初始化二级菜单项中的链接项内容
	function initSecondLevelMenuItemDialog() {
		secondLevelMenuItemDialog = $("#secondLevelMenuItemDialog").dialog({
			autoOpen : false,
			title : "添加二级菜单项",
			...
			buttons : {
				"保存" : function() {
					saveSecondLevelMenuItem();
				},
				"取消" : function() {
					secondLevelMenuItemDialog.dialog("close");
				}
			},
			close : function() {
				secondLevelMenuItemProxy.hideMessages();
			}
		});
	}
	
	// 检验二级菜单项
	function checkSecondLevelMenuItem() {
		// 防缓存
		secondLevelMenuItemProxy = FormProxy.newOne();
		
		secondLevelMenuItemProxy.addField("menuId");
		secondLevelMenuItemProxy.addField("curFirstLevelMenuItemId");
		secondLevelMenuItemProxy.addField({
			id : "secondLevelMenuItemName",
			required : true,
			rules : ["rangeLength[1, 30]"]
		});
		secondLevelMenuItemProxy.addField({
			id : "secondLevelMenuItemHasUrl",
			required : true
		});
		secondLevelMenuItemProxy.addField({
			id : "secondLevelMenuItemUrl",
			get : function(idOrName, type, curData, asRawVal) {
				// 若“包含链接”项被选择，取对应的url值；否则返回""
				var hasUrl = secondLevelMenuItemProxy.getValue("secondLevelMenuItemHasUrl");
				if(hasUrl == 1) {
					return $id("secondLevelMenuItemUrl").val();
				}
				return "";
			},
			rules : ["rangeLength[0, 250]", {
		...
		console.log(objToJsonStr(secondLevelMenuItemProxy.getValues()));
		return secondLevelMenuItemProxy.validateAll();
	}
	
	// 添加二级菜单项
	function saveSecondLevelMenuItem() {
		if (!checkSecondLevelMenuItem()) {
			return;
		}
	
		var ajax = Ajax.post("/categ/menu/item/second/level/add/do");
		ajax.data(secondLevelMenuItemProxy.getValues());
		ajax.done(function(result, jqXhr) {
			if (result.type == "info") {
				Layer.msgSuccess(result.message);
				// 更新二级菜单列表、加载三级菜单列表
				loadSecondLevelMenuItems(secondLevelMenuItemProxy.getValue("curFirstLevelMenuItemId"));
	...
	function loadTrirdLevelMenuItems(pId, _this) {
		$("#secondLevelMenuItem .lable.active").each(function(i,obj){
			$(obj).removeClass("lable active");
		});
		$(_this).parent().addClass("lable active");
...
// 									+ "<button class='normal button' style='width: 20px;' onclick='moveOrder(1)'>↓</button>";
	...
	// 关联删除商品分类二级菜单
	function delSecondLevelMenuItem(id) {
		var theLayer = Layer.confirm('删除二级菜单项将删除该菜单项下面的所有关联的菜单项，确定吗？', function() {
			var hintBox = Layer.progress("正在删除...");
			var ajax = Ajax.post("/categ/second/level/menu/item/del/do");
			ajax.data({
				id : parseInt(id)
			});
			ajax.done(function(result, jqXhr) {
				if (result.type == "info") {
					...
					// 刷新数据
					loadSecondLevelMenuItems($id("curFirstLevelMenuItemId").val());
				} else {
					Layer.msgWarning(result.message);
				}
			});
	...
	// type 0：上移 1：下移
// 	function moveOrder(type) {
// 		var order = [];
// 		var num = thirdLevelMenuItemGrid.getDataIDs().length;
// 		var id = thirdLevelMenuItemGrid.jqGrid("getGridParam", "selrow");
// 		var cellIndex = thirdLevelMenuItemGrid.jqGrid("getCell", id, 0);
// 		if (type == 0) {
// 			if (cellIndex == 0) {
// 				return;
// 			}
// 			order.add(cellIndex);
// 			order.add(cellIndex - 1);
// 			thirdLevelMenuItemGrid.remapColumns(order, true, false);
// 		} else if (type == 1) {
// 			if (cellIndex == num) {
// 				return;
// 			}
...
// 	}
	...
	function addMenuItemCat() {
		if (!$id("curSecondLevelMenuItemId").val()) {
			...
			var data = {
				id : null,
				menuItemId : $id("curSecondLevelMenuItemId").val(),
				catFlag : false,
				catId : null,
				name : null,
				linkUrl : null,
	...
	function saveMenuItemCat() {
		...
		ajax.done(function(result, jqXhr) {
			if (result.type == "info") {
				Layer.msgSuccess(result.message);
				loadTrirdLevelMenuItems($id("curSecondLevelMenuItemId").val());
			} else {
				Layer.msgWarning(result.message);
			}
		});
	...
	$(function() {
		...
		// 初始化二级菜单项的链接项对话框
		initSecondLevelMenuItemDialog();
		// 添加二级菜单项
		$("#btnAddSecondLevelMenuItem").click(function() {
			$id("secondLevelMenuItemName").val('');
			$id("secondLevelMenuItemHasUrlNo").attr("checked","checked");
			$id("secondLevelMenuItemUrl").val('');
			secondLevelMenuItemDialog.dialog("open");
		});
...
<script id="firstLevelGoodsCategMenuTpl" type="text/html" title="商品分类一级菜单">
	{{# for(var i = 0, len = d.length; i < len; i++){ }}
		<div class="field row">
			<div class="normal group left aligned">
				<label class="field label two wide align left" style="cursor: pointer;" onclick="loadSecondLevelMenuItems({{ d[i].id }}, this);$id('curFirstLevelMenuItemId').val({{ d[i].id }});">{{ d[i].name }}</label>
			</div>
			<div class="normal group right aligned">
...
<script id="secondLevelGoodsCategMenuTpl" type="text/html" title="商品分类二级菜单">
	{{# for(var i = 0, len = d.length; i < len; i++){ }}
		<div class="field row">
			<div class="normal group left aligned">
				<label class="field label two wide align left" style="cursor: pointer;" onclick="loadTrirdLevelMenuItems({{ d[i].id }}, this);$id('curSecondLevelMenuItemId').val({{ d[i].id }});">{{ d[i].name }}</label>
			</div>
			<div class="normal group right aligned">
				<!-- <button class="normal button" style="width: 20px;">↑</button>
				<button class="normal button" style="width: 20px;" name="arrowDown">↓</button> -->
				<button class="normal button" style="width: 50px;" onclick="delSecondLevelMenuItem({{ d[i].id }});">删除</button>
			</div>
		</div>
	{{# } }}
</script>

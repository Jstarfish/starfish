...
<body id="rootPanel">
	...
	<div id="mainPanel" class="ui-layout-center" style="padding: 0px;">
		<div id="tabs" class="noBorder">
				...
				<div class="filter section">
					...
					<div class="field row">
						<div class="field group">
							...
							<input id="goodsCat" data-hasSpec="" data-id="" data-parentId="" data-idPath="" readonly="readonly" style="line-height: 28px;" placeholder="请选择..." class="field two wide value" />
						</div>
						<div class="field group">
			...
			<div id="tab_intro">
				<textarea id="goods_intro_editor">
								
				</textarea>
			</div>
			<div style="display:none;" id="tab_relatedGoods">
			</div>
			<div style="display:none;" id="tab_parts">
			</div>
	...
	$(function() {
		...
		//加载tab页面
		jqTabsCtrl = $("#tabs").tabs({ disabled: [1, 2, 3, 4, 5] });
		//
		initCategTreeDialog();
		//初始化商品分类
		initGoodsCat();
	...
	function saveBasisInfo(){
		...
		var ajax = Ajax.post("/goods/basisInfo/save/do");
		var basisInfoMap = getBasisInfoMap();
		ajax.data(basisInfoMap);
		ajax.done(function(result, jqXhr) {
			if (result.type == "info") {
				...
				//
				//是否启用规格标识
				currentCategHasSpec = goods.hasSpec;
				//
				refreshGoods(goods, true);
				var vendorId = goods.vendorId;
				currentVendorId = vendorId;
				//
				Layer.msgSuccess(result.message);
				//
				jqTabsCtrl.tabs('enable');
	...
	//
	function goSaveBasisInfo(){
		var vldResult = validateBasisInfo();
		if(vldResult) saveBasisInfo();
	}
	//
	...
	function validateBasisInfo() {
		...
		//
		if (vldResult) {
			$id("keyAttr_tab").find("[key-flag='true']").each(function() {
				if (isNoB($(this).val())) {
	...
	function getProductMap(vid) {
		...
		productMap["shelfStatus"] = shelfStatus;
	...
	function saveGoodsColorImgs(){
		...
		console.log("_goodsColorImgs="+JSON.encode(_goodsColorImgs));
	...
	function addFreeCombProducts(specCartList){
		if(specCartList.length > 6){
			Toast.show("亲，当前操作创建过多产品哦！建议规格量减少~", 3000, "warning");
		...
		$(tableObj).find("button[data-action='openImgDlg']").click(function() {
			...
			dlgConfig.okClickHandler = function(imageDlg) {
				...
				for (var i = imageItems.length-1; i >= 0; i--) {
					...
					console.log("item=" + JSON.encode(item));
				}
	...
	//初始化商品相册选择颜色分组事件
	function initGoodsColorImgHandler(){
		//
	...
	function clickNode(e, treeId, treeNode) {
		...
		//
		$id("goodsCat").attr("data-id", currentCatId);
	}
	//
	var setting = {
		view : {
